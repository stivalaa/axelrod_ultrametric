...
#define SVN_VERSION_MODEL_CPP "Derived from Rev 942: MODIFIED-SRAND-TMP THETA"
...
void model(Grid<double>& G, Grid<int>& L, Grid<int>& O, Grid<int>& C, int tmax, int n, int m, int F,
    ...
    for (int t = 0; t < tmax; t++) {
	  ...
	  G.write((toString(tempfileprefix) + "-" + toString(timesteps[nextstep]) + ".adj").c_str(), ' ');
...
int main(int argc, char* argv[]) {
   ...
   double theta = 0.0;
	...
	// Call the model
    model(G, L, O, C, tmax, n, m, F, q, r, s, toroidal, network, tolerance, directed_migration,
