...
stats = c("n", "m", "beta_s", "q", "run", "time", "num_communities", "largest_community", "between_community_diversity")
...
D <- D[stats]
D <- melt(D, id=c("n", "m", "beta_s", "q", "run", "time"))


D2 <- summaryBy(value ~ n + m + beta_s + q + variable, data=D, FUN=c(mean)) 
...
D3 <- summaryBy(value ~ n + m + beta_s + q + variable, data=D, FUN=c(mean, sd)) 
...
Drect = data.frame(n = rep(ns, length(ms)), m = rep(ms, each=length(ns)), left = rep(ns, length(ms)), right = rep(ns, length(ms)))
for (k in ms) {
  for (l in ns) {
    tmp <- D2[which(D2$variable == "between_community_diversity" & D2$n == l & D2$m == k & D2$blub == 1),]
    Drect$left[which(Drect$n == l & Drect$m == k)] <- tmp$q[which.min(tmp$q)]
    Drect$right[which(Drect$n == l & Drect$m == k)] <- tmp$q[which.max(tmp$q)]
  }
}


Dline = data.frame(n = rep(ns, length(ms)), m = rep(ms, each=length(ns)), loc = rep(ns, length(ms)))
Dline$m <- as.factor(Dline$m)
for (k in ms){
  for (l in ns) {
    tmp <- D3[which(D3$n == l & D3$m == k),]
    Dline$loc[which(Dline$n == l & Dline$m == k)] <- tmp$q[which.max(tmp$value.sd)]
  }
}
...
pdf(paste("img/qc.pdf", sep=''))
p <- ggplot(Dline, aes(x=n, y=loc, shape=m, colour=m, linetype=m))
p <- p + geom_line() + geom_point(size=1.6)

p <- p + scale_x_continuous(labels = comma, breaks=c(0.04, 0.2, 0.4, 0.8, 1.0))
p <- p + xlab("$N/L^2$") + ylab("$q_c$")
p <- p + theme_bw()
...
p <- p + scale_colour_brewer("$L$", palette = "Set1")
p <- p + scale_linetype("$L$")
p <- p + scale_shape("$L$")
print(p)
...
D3 <- ddply(D3, .(n,m), function(x)x[x$value.sd==max(x$value.sd), ])
Dline <- D3[, c("n", "m", "q")]

Dline$m <- factor(Dline$m)


#tikz(paste("img/qc-alt.tex", sep=''), standAlone = TRUE, width=3, height=2, documentDeclaration = "\\documentclass[final,times]{elsarticle}")
# PDF, don't use postscript as shading doesn't work
pdf(paste("img/qc-alt.pdf", sep=''))
p <- ggplot(Dline, aes(x=n, y=loc, shape=m, colour=m, linetype=m))
p <- ggplot(Dline, aes(x=n, y=q, shape=m, colour=m))
p <- p + geom_point(size=1.6)

p <- p + xlab(expression(N)) + ylab(expression(q[c]))
p <- p + theme_bw()
p <- p + opts(plot.margin = 	    unit(c(0,0,0,0), "lines"),
	      axis.text.x =       theme_text(angle = 90, size = 8, colour = "black", lineheight = 0.2),
	      axis.text.y =       theme_text(size = 8, colour = "black", lineheight = 0.2),
...
p <- p + scale_colour_brewer(expression(L), palette = "Set1")
p <- p + scale_linetype(expression(L))
p <- p + scale_shape(expression(L))

p <- p + geom_segment(data=Dline[which((Dline$n / (as.numeric(as.character(Dline$m)) * as.numeric(as.character(Dline$m)))) == 1),], aes(x = n, y=q, xend=n, yend=0), linetype=3, colour="black")
print(p)
