...
bla <- function(variable, value) {
    ...
    plyr::llply(value, function(x) paste(variable, 
        " = ", x))
}
...
ns = c(25,125,500,625)
beta_ss <- c(1,10)
stats = c("n", "beta_s", "q", "theta", "init_random_prob", "run", "time", "num_communities", "within_community_diversity", "between_community_diversity", "num_cultures")

responses=c("Number of communities", "Intra-community diversity", "Inter-community diversity", "Number of cultures")
...
D <- summaryBy(value ~ n + beta_s + q + time + variable, data=D, FUN=c(mean,sd)) 
...
for (r in 1:length(responses)) {
  ...
  postscript(paste("img/time-", stats[r+7], ".eps", sep=''), 
  ...
  p <- ggplot(Dst, aes(x=time, y=value.mean, colour=q, linetype=q))
