...
for (r in 1:length(responses)) {
  ...
  postscript(paste("img/initcultureconnectedcomponents-theta-", stats[r+10], ".eps", sep=''), 
...
#  limits <- aes(ymax = Dst$value.mean + Dst$value.sd, ymin=Dst$value.mean - Dst$value.sd) 
  p <- ggplot(Dst, aes(x=initial_num_culture_components, y=value, colour=as.factor(theta))) #+ opts(title=bquote(q == .(qs[1])))
  p <- p + geom_point()
  ...
  p <- p + ylab(response)  
  p <- p + facet_grid(initial ~ beta_s , labeller =  bla)
  p <- p + theme_bw()
