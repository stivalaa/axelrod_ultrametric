...
#
# This version uses facets to plot cophenetic cc against time with different
# values of init_random_prob and q, with N and beta_s fixed rather than
# having N/L^2 and beta_s as facets.
#
# Usage:
#
...
#
#
# Output .eps file in img/ has filename time-theta-initrandom-prob-ultrametricity.csv
#
# ADS February 2013
    ...
    sapply      (value, FUN=function(xval ) 
        if (variable == "beta_s") {
          ...
          bquote(N/L^2==.(levels(xval)[xval]))
            ...
            paste("Init. perturb prob. = ", format(levels(xval)[xval]))
        }
        else {
...
init_random_probs <- c(0.0, 0.2, 0.4, 0.6, 0.8)
...
D <- summaryBy(value ~ n + beta_s + theta + init_random_prob + q +F + time + variable, data=D, FUN=c(mean,sd)) 
...
my.theme$layout.widths <- list(
  ...
  postscript("img/time-theta-initrandomprob-ultrametricity.eps",
  ...
  p <- ggplot(Dst, aes(x=time, y=value.mean, colour=as.factor(theta), linetype=as.factor(theta))) + opts(title=bquote(list(N/L^2 == .(levels(D$n)[D$n]), beta[s] == .(beta_ss[1]))))
  ...
  p <- p + ylab(response)  
  p <- p + facet_grid(q ~ init_random_prob, labeller = bla)
  p <- p + theme_bw()
