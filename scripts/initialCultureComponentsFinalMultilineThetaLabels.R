...
# Usage:
#
# Rscript initialCultureComponentsFinalMultilineThetaLabels.R results_csv_filename.csv
#
# (or R -f initialCultureComponentsFinalMultilineThetaLabels.R --args results_csv_filenme.csv)
#
# E.g.:
#  Rscript  ../../scripts//initialCultureComponentsFinalMultilineThetaLabels.R multiendresults.csv 
#
# Output .eps file in img/ has filename based on
# type of graph, with -nXXX for value of n, e.g.
#      initcultureconnectedcomponents-multilineThetaLabels-num_cultures-n125.csv
#
# ADS December 2013
...
#D <- summaryBy(value ~ n + beta_s + q + theta + init_random_prob + initial_num_culture_components + variable, data=D, FUN=c(mean,sd)) 
  ...
  for (r in 1:length(responses)) {
    ...
    postscript(paste("img/initcultureconnectedcomponents-multilineThetaLabels-", stats[r+10], '-n', n, ".eps", sep=''), 
...
#  limits <- aes(ymax = Dst$value.mean + Dst$value.sd, ymin=Dst$value.mean - Dst$value.sd) 
    p <- ggplot(Dst, aes(x=initial_num_culture_components, y=value, colour=as.factor(init_random_prob), label=theta, linetype=as.factor(init_random_prob))) 
    p <- p + geom_text(aes(label=theta))
