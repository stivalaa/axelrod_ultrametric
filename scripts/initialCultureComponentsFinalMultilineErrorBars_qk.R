...
# This version uses facets with q and k (prototype initial conditions number
# of prototypes - so only for prototype_init_culture:k runs), so F and n 
# are fixed.
#
# Usage:
#
# Rscript initialCultureComponentsFinalMultilineErrorBars_qk.R results_csv_filename.csv
#
# (or R -f initialCultureComponentsFinalMultilineErrorBars_qk.R --args results_csv_filenme.csv)
#
# E.g.:
#  Rscript  ../../scripts//initialCultureComponentsFinalMultilineErrorBars_qk.R multiendresults.csv 
#
...
#beta_ss <- c(1,10)
beta_ss <- c(1)
stats <- c("k", "n", "m", "F", "beta_p","init_random_prob", "beta_s", "q", "theta", "run", "time", "num_communities", "within_community_diversity", "between_community_diversity", "num_cultures", "num_components", "largest_component" , "initial_num_culture_components")
...
for (n in ns) {
  ...
  idcolnames <- c("k", "n", "m", "F", "beta_p", "beta_s", "q", "theta",  "run", "init_random_prob")
...
###D$init_random_prob < factor(D$init_random_prob)
  ...
  D <- summaryBy(value ~ k + n + beta_s + F + q + theta + init_random_prob + initial_num_culture_components + variable, data=D, FUN=c(mean,sd)) 
  ...
  for (r in 1:length(responses)) {
    ...
    p <- p + ylab(response)  
    p <- p + facet_grid(q ~ k , labeller =  bla)
    p <- p + theme_bw()
