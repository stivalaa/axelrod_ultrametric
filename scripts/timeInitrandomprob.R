...
# type of graph e.g. time-initrandomprob-num_communities.eps
#
# ADS February 2013
...
#ns = c(6000)
beta_ss <- c(1,10)
stats = c("n", "beta_s", "q", "theta", "init_random_prob", "run", "time", "num_communities", "within_community_diversity", "between_community_diversity", "num_cultures", "ass", "cluster_coeff", "social_clustering", "avg_degree")

responses=c("Number of communities", "Intra-community diversity", "Inter-community diversity", "Number of cultures", "Assortativity", "Clustering coefficient", "Social_clustering", "Mean degree")
...
D <- D[stats]
D <- melt(D, id=c("n", "beta_s", "q", "theta", "init_random_prob", "run", "time"))
...
D <- summaryBy(value ~ n + beta_s + init_random_prob + time + variable, data=D, FUN=c(mean,sd)) 
...
for (r in 1:length(responses)) {
  ...
  postscript(paste("img/time-initrandomprob-", stats[r+7], ".eps", sep=''), 
  ...
  p <- ggplot(Dst, aes(x=time, y=value.mean, colour=as.factor(init_random_prob), linetype=as.factor(init_random_prob))) + opts(title=bquote(list(q == .(qs[1]), theta == .(thetas[1]))))
