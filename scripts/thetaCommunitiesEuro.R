...
#ns <- c(60)
beta_ss <- c(1,10)
stats <- c("initial", "n", "m", "beta_s", "q", "theta", "run", "time", "num_communities", "within_community_diversity", "between_community_diversity", "num_cultures", "num_components", "largest_component", "ass", "cluster_coeff", "social_clustering" , "avg_degree")
...
#initials <- c("real", "permuted", "random")

# read the results file and reduce to columns that we are interested in
D <- read.table("multiendresults.csv", header=TRUE, sep=",",stringsAsFactors=FALSE)
...
#D <- D[which(D$n %in% ns),]
#D <- D[which(D$initial %in% initials),]
...
D <- D[stats]
D <- melt(D, id=c("n", "m", "beta_s", "q", "theta", "run", "time", "initial"))
...
D <- summaryBy(value ~ n + m  + beta_s + theta + initial + variable, data=D, FUN=c(mean,sd)) 
...
for (r in 1:length(responses)) {
  ...
  postscript(paste("img/theta-", stats[r+8], ".eps", sep=''), 
  ...
  p <- ggplot(Dst, aes(x=theta, y=value.mean, colour=as.factor(initial), linetype=as.factor(initial) ))
  p <- p + geom_line() + geom_errorbar(aes(ymin=value.mean - value.sd, ymax=value.mean + value.sd), width=0.1)
  print(paste("XXXXXXX ", response))#XXX
  ...
  p <- p + ylab(response)  
  p <- p + facet_grid(beta_s ~ m, labeller = bla)
  #p <- p + facet_grid(beta_s ~ m, labeller =  label_bquote(beta[s] == .(x)))
  p <- p + theme_bw()
