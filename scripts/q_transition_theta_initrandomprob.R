...
#
# This version plots critical value of q depending on theta, with different
# lines on same plot for initial pertubation probability, rather than F.
#
# Usage:
#
# Rscript q_transitino_theta_init_random_prob.R results_csv_filename.csv
#
#
# Output .pdf file in img/ is qc-theta-initrandomprob.pdf
#
# Also makes qc-theta-initrandomprob-F5.pdf as a separate file as this
# is the most useful plot (higher values of F not so meaningful,
# see output of q_transition2.R)
#
# ADS 4April2013
# 
...
#D <- D[which(D$q %in% qs),]
  ...
  Dline = data.frame(theta = rep(thetas, length(init_random_probs)), init_random_prob = rep(init_random_probs, each=length(thetas)), loc = rep(thetas, length(init_random_probs)))
  Dline$init_random_prob <- as.factor(Dline$init_random_prob)
  for (k in init_random_probs){
    for (l in thetas) {
      tmp <- D3[which(D3$theta == l & D3$init_random_prob == k),]
      qmin <- tmp$q[which.max(tmp$value.sd)]
      if (length(qmin) == 0) { # catches integer(0) value when no min sd
        cat('warning: no min sd of largeust_community for theta = ', l, ' init_Random_prob = ', k, ' F = ', F, '; skipped\n')
        next
      }
      Dline$loc[which(Dline$theta == l & Dline$init_random_prob == k)] <- qmin
    }
  }
  ...
  p <- ggplot(Dline, aes(x=theta, y=loc, shape=init_random_prob, colour=init_random_prob, linetype=init_random_prob))

  p <- p + opts(title=bquote(list(F == .(levels(D$F)[D$F]), N/L^2 == .(noverlsquared))))
...
#		panel.background =  theme_rect(colour = "grey80"),
  ...
  p <- p + scale_shape(expression(p))

  if (F %in% useful_Fs) {
    pdf(paste("img/qc-theta-initrandomprob-F", F, ".pdf", sep=''))
    print(p)
    dev.off()
  }

  # set all to same scale on multiplot page to avoid misleading comparisons
  p <- p + scale_y_continuous(limits=c(2,100))

  plotlist <- c(plotlist, list(p))

}
# PDF suitable for inserting into LaTeX, don't use postscript as shading doesn't work
pdf("img/qc-theta-initrandomprob.pdf")
