...
# Usage:
#
# Rscript initialCultureComponentsFinalMultiline.R results_csv_filename.csv
#
# (or R -f initialCultureComponentsFinalMultiline.R --args results_csv_filenme.csv)
#
# E.g.:
#  Rscript  ../../scripts//initialCultureComponentsFinalMultiline.R multiendresults.csv 
#
# Output .eps file in img/ has filename based on
# type of graph, with -nXXX for value of n, e.g.
#      initcultureconnectedcomponents-multiline-num_cultures-n125.csv
#
# ADS February 2013
...
###D$init_random_prob < factor(D$init_random_prob)
  ...
  D <- summaryBy(value ~ n + beta_s + q + F + theta + init_random_prob + initial_num_culture_components + time + variable, data=D, FUN=c(mean,sd)) 
  ...
  for (r in 1:length(responses)) {
    ...
    postscript(paste("img/initcultureconnectedcomponents-multiline-", stats[r+10], '-n', n, ".eps", sep=''), 
    ...
    cat('\n')
    cat(paste(format(Dst$beta_s,digits=2),format(Dst$init_random_prob,digits=2), format(Dst$theta,digits=3,nsmall=2),format(Dst$initial_num_culture_components, digits=4,nsmall=3),format(Dst$time, scientific=TRUE,nsmall=2), format( Dst$value.mean, digits=4,nsmall=3), sep='\t'), sep='\n')
    cat('\n\n')

#  limits <- aes(ymax = Dst$value.mean + Dst$value.sd, ymin=Dst$value.mean - Dst$value.sd) 
    p <- ggplot(Dst, aes(x=initial_num_culture_components, ymax=value.mean + value.sd, ymin=value.mean - value.sd, colour=as.factor(init_random_prob), linetype=as.factor(init_random_prob))) 
