...
ns = c(25,125,500,625)
beta_ss <- c(1,10)
stats <- c("n", "beta_s", "q", "theta", "run", "time", "num_communities", "within_community_diversity", "between_community_diversity", "num_cultures", "num_components", "largest_component", "ass", "cluster_coeff", "social_clustering","avg_degree" )
...
D <- read.table("endresults.csv", header=TRUE, sep=",",stringsAsFactors=FALSE)
D <- D[which(D$q %in% qs),]
...
D <- D[stats]
D <- melt(D, id=c("n", "beta_s", "q", "theta", "run", "time"))
...
D <- summaryBy(value ~ n + beta_s + q + theta + variable, data=D, FUN=c(mean,sd)) 
...
for (r in 1:length(responses)) {
  ...
  postscript(paste("img/theta-", stats[r+6], ".eps", sep=''), 
  ...
  p <- ggplot(Dst, aes(x=theta, y=value.mean, colour=as.factor(q), linetype=as.factor(q))) #+ opts(title=bquote(N/L^2 == .(noverlsquared)))
