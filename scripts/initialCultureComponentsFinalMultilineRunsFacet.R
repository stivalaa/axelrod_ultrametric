...
# Usage:
#
# Rscript initialCultureComponentsFinalMultilineRunsFacet.R results_csv_filename.csv
#
# (or R -f initialCultureComponentsFinalMultilineRunsFacet.R --args results_csv_filenme.csv)
#
# E.g.:
#  Rscript  ../../scripts//initialCultureComponentsFinalMultilineRunsFacet.R multiendresults.csv 
#
# Output .eps file in img/ has filename based on
# type of graph, with -nXXX for value of n, e.g.
#      initcultureconnectedcomponents-multilinerunsfacet-num_cultures-n125.csv
#
# This version has a facet with run number to show results on different
# runs since putting error bars etc. with multiple rusn on single graphs
# amkes too much of a mess (see initialCultureComponentsFinalMultilineMultiRuns.R)
# ADS February 2013
...
# only get one run 
# (use initialCultureComponentsFinal.R to get scatter plot with multiple runs)
###  D <- D[which(D$run == 0),]

  # need to limit number of runs, 50 is way to many to fit on page
  runs <- c(10, 20, 30, 40, 50)
  D <- D[which(D$run %in% runs), ]

# for this version get only a single value of F as run now one fact
  D <- D[which(D$F == 100), ]
...
#D <- summaryBy(value ~ n + beta_s + q + theta + init_random_prob + initial_num_culture_components + variable, data=D, FUN=c(mean,sd)) 
  ...
  for (r in 1:length(responses)) {
    ...
    postscript(paste("img/initcultureconnectedcomponents-multilinerunsfacet-", stats[r+10], '-n', n, ".eps", sep=''), 
    ...
    cat('beta_s','init_random_prob','theta','intiial_num_culture_components','time',response,sep='\t')
    cat('\n')
    cat(paste(format(Dst$beta_s,digits=2),format(Dst$init_random_prob,digits=2), format(Dst$theta,digits=3,nsmall=2),format(Dst$initial_num_culture_components, digits=4,nsmall=3),format(Dst$time, scientific=TRUE,nsmall=2), format( Dst$value, digits=4,nsmall=3), sep='\t'), sep='\n')
    cat('\n\n')

#  limits <- aes(ymax = Dst$value.mean + Dst$value.sd, ymin=Dst$value.mean - Dst$value.sd) 
    p <- ggplot(Dst, aes(x=initial_num_culture_components, y=value, colour=as.factor(init_random_prob), linetype=as.factor(init_random_prob))) 
    p <- p + opts(title=bquote(list(N/L^2 == .(noverlsquared), beta[s] == .(beta_ss[1]), F == .(levels(Dst$F)[Dst$F])                )))
    p <- p + geom_point()
    ...
    p <- p + ylab(response)  
    p <- p + facet_grid(q ~ run , labeller =  bla)
    p <- p + theme_bw()
