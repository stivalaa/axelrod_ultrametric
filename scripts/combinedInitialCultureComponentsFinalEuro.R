...
# Usage:
#
# Rscript combinedIntialFinalInitRandomProbCultures 
#
# Output .eps file is
#
#  combined-euro-gss-initcultureconnectedcomponents-num_cultures.csv
#
# This version is for Eurobarometer data: fixed values of n, q
# and with real and siulated eurobarometer and gss data combined on one
# single page to try to fit in 2 column journal format
#
# ADS August 2013
...
results_filenames <- c('../../axelrod_merged_eurobarometer_reults/50samples/multiendresults.csv',
                       '../../axelrod_merged_gss_results/600samples/multiendresults.csv',
                       '../../axelrod_merged_simulated_eurobarometer_reults/50samples/multiendresults.csv',
                       '../../axelrod_merged_simulated_gss_results/600samples/multiendresults.csv')
results_names <- c('Eurobarometer', 'GSS', 'Simulated Eurobarometer', 'Simulated GSS')


#ns = c(25,125,500,625)
#beta_ss <- c(1,10)
beta_ss <- c(1)
stats <- c("n", "m", "F", "beta_p","initial", "beta_s", "q", "theta", "run", "time", "num_communities", "within_community_diversity", "between_community_diversity", "num_cultures", "num_components", "largest_component" , "initial_num_culture_components")
...
for (resindex in 1:length(results_filenames)) {
  results_filename <- results_filenames[resindex]
...
#  D <- D[which(D$theta %in% thetas),]
  ...
  D <- summaryBy(value ~ n + m + beta_s + q + theta + initial + initial_num_culture_components + variable, data=D, FUN=c(mean,sd)) 
  ...
  for (r in 1:length(responses)) {
    ...
    if (response != 'Number of cultures') {
      next
    }
    ...
    cat('beta_s','initial','theta','intiial_num_culture_components','time',response,sep='\t')
    cat('\n')
    cat(paste(format(Dst$beta_s,digits=2),format(Dst$initial,width=12), format(Dst$theta,digits=3,nsmall=2),format(Dst$initial_num_culture_components, digits=4,nsmall=3),format(Dst$time, scientific=TRUE,nsmall=2), format( Dst$value, digits=4,nsmall=3), sep='\t'), sep='\n')
    cat('\n\n')

  #  limits <- aes(ymax = Dst$value.mean + Dst$value.sd, ymin=Dst$value.mean - Dst$value.sd) 
    p <- ggplot(Dst, aes(x=initial_num_culture_components, y=value.mean, colour=as.factor(initial), linetype=as.factor(initial))) #+ opts(title=bquote(q == .(qs[1])))
    p <- p + opts(title=results_names[resindex])
    ...
    p <- p + xlab("Initial connected cultural components")
    p <- p + ylab(response)  
#    p <- p + facet_grid(m ~ beta_s , labeller =  bla)
    p <- p + theme_bw()
    p <- p + opts(plot.margin = 	    unit(c(0,0,0,0), "lines"),
                  axis.text.x =       theme_text(size = 11, colour = "black", lineheight = 0.2),
                  axis.text.y =       theme_text(size = 11, colour = "black", lineheight = 0.2),
                  axis.title.x =       theme_text(size = 11, colour = "black", lineheight = 0.2),
                  axis.title.y =       theme_text(angle = 90, size = 11, colour = "black", lineheight = 0.2),

                strip.text.x =	theme_text(size = 11, colour = "black"),
                strip.text.y =	theme_text(angle = -90, size = 11, colour = "black"),
                legend.text = 	theme_text(size = 11, colour = "black"),
                legend.title = 	theme_text(size = 11, face = "bold", colour = "black"),
    ...
    # put legend at bottom right except Simulated Eurbarometer where it
    # collides with lowest curve a bit so bit in top left instead
    if (results_names[resindex] == "Simulated Eurobarometer") {
      # (0,1) does not work, got this by trial&error
      p <- p + theme(legend.position=c(0.55,0.45), legend.justification=c(1,0))
    }
    else {
...
postscript('combined-euro-gss-initcultureconnectedcomponents-num_cultures.eps',
