...
# This version uses facets with q and ef2_r (neutral evolution initial conditions number
# of traits to change- so only for evolved_init_culture2:r runs), so F and n 
# are fixed.
#
# Usage:
#
# Rscript initialCultureComponentsFinalMultilineErrorBars_qr.R results_csv_filename.csv
#
# (or R -f initialCultureComponentsFinalMultilineErrorBars_qr.R --args results_csv_filenme.csv)
#
# E.g.:
#  Rscript  ../../scripts//initialCultureComponentsFinalMultilineErrorBars_qr.R multiendresults.csv 
#
...
#beta_ss <- c(1,10)
beta_ss <- c(1)
stats <- c("ef2_r", "n", "m", "F", "beta_p","init_random_prob", "beta_s", "q", "theta", "run", "time", "num_communities", "within_community_diversity", "between_community_diversity", "num_cultures", "num_components", "largest_component" , "initial_num_culture_components")
...
for (n in ns) {
  ...
  D <- D[which(D$ef2_r %in% ef2_rs),]

  D <- D[which(D$n == n),]
  D$n <- D$n / (D$m * D$m)
  noverlsquared <- D$n

  idcolnames <- c("ef2_r", "n", "m", "F", "beta_p", "beta_s", "q", "theta",  "run", "init_random_prob")
...
# now get rid of the time=0 rows
  ...
  D$q <- factor(D$q)
  D$F <- factor(D$F)
  D$ef2_r <- factor(D$ef2_r)
  D$beta_s <- factor(D$beta_s)
###D$init_random_prob < factor(D$init_random_prob)
  D$variable <- factor(D$variable, labels=responses)

  D <- summaryBy(value ~ ef2_r + n + beta_s + F + q + theta + init_random_prob + initial_num_culture_components + variable, data=D, FUN=c(mean,sd)) 
  ...
  for (r in 1:length(responses)) {
    ...
    postscript(paste("img/initcultureconnectedcomponents-multilineerrorbars-", stats[r+11], '-n', n, ".eps", sep=''), 
    ...
    p <- p + opts(title=bquote(list(F == .(Fs[1]), N/L^2 == .(noverlsquared), beta[s] == .(beta_ss[1])                )))
    ...
    p <- p + ylab(response)  
    p <- p + facet_grid(q ~ ef2_r , labeller =  bla)
    p <- p + theme_bw()
