...
# initialultrametricity-cophenetic_cc.eps etc. 
# (fixed values of q,F; facets on N/L^2 and theta )
#
# ADS February 2013
...
for (r in 1:length(responses)) {
  ...
  postscript(paste("img/initialultrametricity-", stats[r+10], ".eps", sep=''), 
  ...
  Dst <- D[which(D$variable == response),]
  p <- ggplot(Dst, aes(x=initial_cophenetic_cc, y=value, colour=as.factor(init_random_prob), linetype=as.factor(init_random_prob))) + opts(title=bquote(list(q == .(qs[1]),F == .(Fs[1]), beta[s] == .(beta_ss[1]) )))
  p <- p + geom_point()
  ...
  p <- p + ylab(response)  
  p <- p + facet_grid(theta ~ n, labeller =  bla)
  p <- p + theme_bw()
