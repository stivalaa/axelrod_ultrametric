...
# Usage:
#
# Rscript thetaInitRandomProbCommunities results_csv_filename.csv
#
# (or R -f thetaInitRandomProbCommunities --args results_csv_filenme.csv)
#
# Output .eps file in img/ has filename based on
# type of graph e.g. theta-initrandomprob-num_communities.eps
# (fixed values of q,F; facets on N/L^2 and beta_s)
#
# ADS February 2013
...
for (r in 1:length(responses)) {
  ...
  postscript(paste("img/theta-initrandomprob-", stats[r+8], ".eps", sep=''), 
  ...
  p <- ggplot(Dst, aes(x=theta, y=value.mean, colour=as.factor(init_random_prob), linetype=as.factor(init_random_prob))) + opts(title=bquote(list(q == .(qs[1]),F == .(Fs[1]))))
#  p <- p + geom_point()
