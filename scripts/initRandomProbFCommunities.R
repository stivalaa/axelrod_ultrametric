...
# Usage:
#
# Rscript initRandomProbFCommunities.R results_csv_filename.csv
#
#
# Output .eps file in img/ has filename based on
# type of graph e.g. initrandomprob-F-num_communities.eps
#
# ADS February 2013
...
for (r in 1:length(responses)) {
  ...
  postscript(paste("img/initrandomprob-F-", stats[r+8], ".eps", sep=''), 
  ...
  p <- p + opts(title = bquote(list(theta == .(thetas[1]), beta[s] == .(beta_ss[1]))))
  ...
  p <- p + ylab(response)  
  p <- p + facet_grid(F      ~ n, labeller =  bla)
  p <- p + theme_bw()
